...
```
extensions/pi-telegram-extension-demo/package.json name=@evandrodevbr/oh-my-pi-telegram-extension-demo → @evandrodevbr/oh-my-pi-telegram-extension-demo
...
export default function (pi: ExtensionAPI) {
  const unregister = registerTelegramSection({
    id: "@evandrodevbr/oh-my-pi-telegram-extension-demo",
    label: "🧪 Demo submenu",
    order: 10,
...
**Typed import (preferred):** Extension imports `registerTelegramSection` from `@evandrodevbr/oh-my-pi-telegram/sections`. The function reads from a `globalThis` registry set by `pi-telegram` at startup. In `0.12.0`, package-private `@evandrodevbr/oh-my-pi-telegram/lib/*.ts` deep imports are no longer exported.

**Relative import (local):** When the extension cannot resolve `@evandrodevbr/oh-my-pi-telegram` as an npm package, use the public API membrane via a relative path:
...
Telegram limits `callback_data` to 64 bytes. Full npm names like `@evandrodevbr/oh-my-pi-telegram-explorer` often exceed this. `pi-telegram` maps each registered section to a compact numeric token:
...
- Typed imports → typed import from `@evandrodevbr/oh-my-pi-telegram/sections`
...
[`@evandrodevbr/oh-my-pi-telegram-extension-demo`](https://github.com/llblab/pi-telegram-extension-demo) is the maintained companion-extension reference:

- Main-menu and Settings surfaces with dynamic labels.
- Managed section callbacks, buttons, edits, navigation, and cleanup.
- Public `@evandrodevbr/oh-my-pi-telegram/*` imports rather than package-private `/lib` paths.
