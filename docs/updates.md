...
- The registry lives on `globalThis`. Module instance identity is not required, so layered extensions can reach it without importing `@evandrodevbr/oh-my-pi-telegram`.
...
### Typed import (recommended when you can depend on `@evandrodevbr/oh-my-pi-telegram`)
