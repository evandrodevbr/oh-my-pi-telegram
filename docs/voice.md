...
Import provider APIs from `@evandrodevbr/oh-my-pi-telegram/voice`; see the TSDoc on `registerTelegramVoiceSynthesisProvider` and `TelegramVoiceSynthesisProviderResult` there for the exact interface.
...
```typescript
import { recordTelegramRuntimeEvent } from "@evandrodevbr/oh-my-pi-telegram/outbound";
